          ...
          <div class="row no-print">
            <div class="col-lg-12">
              <button type="button" class="btn btn-primary pull-right finish" style="margin-right: 5px;" data-id="{{ $mohon->id }}" data-file="finish_{{ $mohon->id }}"> <i class="fa fa-credit-card"></i> Submit </button>
                {{ Form::open(['url' => route('admin.permohonanverifikasi.kirim', [Crypt::encrypt($mohon->id)]), 'method' => 'get', 'id' => 'finish_'.$mohon->id ]) }}
                {{ csrf_field() }}
                 <input class="form-control" type="hidden" name="jenis" value="rekening">
                {{ Form::close() }}
            </div>
          </div>
        </div>
